...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X_train, y)):
    ...
    print(f"Fold {fold+1}: F1={fold_f1:.4f}, n_iter={model.n_iter_[0]}")
...
np.save("models/logreg_v3_oof.npy", oof_preds)
np.save("models/logreg_v3_test.npy", test_preds)

sub = test[["id"]].copy()
sub["label"] = (test_preds >= best_t).astype(int)
sub.to_csv("submissions/logreg_v3.csv", index=False)
print(f"Saved submissions/logreg_v3.csv, positive rate: {sub['label'].mean():.3f}")
