...
for fold, (tr_idx, val_idx) in enumerate(skf.split(X_train, y)):
    ...
    print(f"Fold {fold+1}: F1={fold_f1:.4f}, best_iter={model.best_iteration_}")
...
np.save("models/lgbm_oof.npy", oof_preds)
np.save("models/lgbm_test.npy", test_preds)

sub = test[["id"]].copy()
sub["label"] = (test_preds >= best_t).astype(int)
sub.to_csv("submissions/lgbm_tfidf.csv", index=False)
print(f"Saved submissions/lgbm_tfidf.csv, positive rate: {sub['label'].mean():.3f}")
