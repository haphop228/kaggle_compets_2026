...
np.save("models/logreg_v5_oof.npy", oof_preds)
np.save("models/logreg_v5_test.npy", test_preds)

sub = test[["id"]].copy()
sub["label"] = (test_preds >= best_t).astype(int)
sub.to_csv("submissions/logreg_v5.csv", index=False)
print(f"Saved submissions/logreg_v5.csv, positive rate: {sub['label'].mean():.3f}")
