...
np.save("models/svc_oof.npy", oof_preds)
np.save("models/svc_test.npy", test_preds)

sub = test[["id"]].copy()
sub["label"] = (test_preds >= best_t).astype(int)
sub.to_csv("submissions/svc_tfidf.csv", index=False)
print(f"Saved submissions/svc_tfidf.csv, positive rate: {sub['label'].mean():.3f}")
