...
for fold, fd in enumerate(fold_data):
    ...
    print(f"[LGB v5] Fold {fold+1}: MAPE={fold_mape:.4f}, best_iter={model.best_iteration_}")

lgb_oof_mape = mean_absolute_percentage_error(np.expm1(y_log), np.expm1(oof_lgb))
print(f"\n[LGB v5] OOF MAPE: {lgb_oof_mape:.4f}")

with open("models/v3_results.pkl", "rb") as f:
    v3 = pickle.load(f)
print(f"[LGB v3] OOF MAPE: {v3['lgb_mape']:.4f}")

for w5 in [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 1.0]:
    blend = w5 * oof_lgb + (1 - w5) * v3["oof_lgb"]
    m = mean_absolute_percentage_error(np.expm1(y_log), np.expm1(blend))
    print(f"  v5={w5:.1f} v3={1-w5:.1f}: MAPE={m:.4f}")

pd.DataFrame({"id": test_ids, "price_target": np.expm1(test_lgb)}).to_csv(
    "submissions/lgbm_v5.csv", index=False)

with open("models/v5_results.pkl", "wb") as f:
    pickle.dump({
        "oof_lgb": oof_lgb,
        "test_lgb": test_lgb,
