...
LOC_DIST_COLS = [
    ...
    "location_highway_crossing_w_mean_distance", "location_shop_alco_w_mean_distance",
...
def base_features(df):
    ...
    # building density / urban features
    ...
    df["infra_score"] = (
        df["location_school_w_mean_distance"].fillna(df["location_school_w_mean_distance"].median()) +
        df["location_amenity_pharmacy_w_mean_distance"].fillna(
            df["location_amenity_pharmacy_w_mean_distance"].median()
        )
    )
    ...
    # min distance to any amenity (non-null)
...
print(f"  LightGBM OOF MAPE:  {lgb_oof_mape:.4f}")
print(f"  XGBoost  OOF MAPE:  {xgb_oof_mape:.4f}")
print(f"  CatBoost OOF MAPE:  {cb_oof_mape:.4f}")
print(f"  Best Blend MAPE:    {best_mape:.4f}")
print(f"  Blend weights: LGB={best_w[0]:.2f} XGB={best_w[1]:.2f} CB={best_w[2]:.2f}")
...
print("Saved models/v3_results.pkl")
