...
for fold, (tr_idx, val_idx) in enumerate(splits):
    ...
    print(f"  [CB] Fold {fold+1}: MAPE={fold_mape:.6f}")

cb_mape = mean_absolute_percentage_error(y_real, np.expm1(oof_cb))
print(f"[CB] OOF MAPE: {cb_mape:.6f}")
...
print(f"Log-space blend: LGB={best_log_w:.2f} CB={1-best_log_w:.2f}, MAPE={best_log_mape:.6f}")

# blend in real-space
oof_lgb_real = np.expm1(oof_lgb)
oof_cb_real = np.expm1(oof_cb)
test_lgb_real = np.expm1(test_lgb)
test_cb_real = np.expm1(test_cb)

best_real_mape = 1.0
best_real_w = 0.8
for w in np.arange(0.0, 1.01, 0.01):
    blend = w * oof_lgb_real + (1 - w) * oof_cb_real
    m = mean_absolute_percentage_error(y_real, blend)
    if m < best_real_mape:
        best_real_mape = m
        best_real_w = w

print(f"Real-space blend: LGB={best_real_w:.2f} CB={1-best_real_w:.2f}, MAPE={best_real_mape:.6f}")

if best_real_mape < best_log_mape:
    final_test = best_real_w * test_lgb_real + (1 - best_real_w) * test_cb_real
    final_mape = best_real_mape
    blend_type = "real-space"
else:
    final_test = np.expm1(best_log_w * test_lgb + (1 - best_log_w) * test_cb)
    final_mape = best_log_mape
    blend_type = "log-space"

print(f"\nFinal ({blend_type}): {final_mape:.6f}")

pd.DataFrame({"id": test_ids, "price_target": np.expm1(test_lgb)}).to_csv(
    "submissions/lgbm_v10.csv", index=False)
pd.DataFrame({"id": test_ids, "price_target": final_test}).to_csv(
    "submissions/blend_v10.csv", index=False)

if blend_type == "real-space":
    log_blend_test = np.expm1(best_log_w * test_lgb + (1 - best_log_w) * test_cb)
    pd.DataFrame({"id": test_ids, "price_target": log_blend_test}).to_csv(
        "submissions/blend_v10_log.csv", index=False)

with open("models/v10_results.pkl", "wb") as f:
    pickle.dump({
        "oof_lgb": oof_lgb, "oof_cb": oof_cb,
        "test_lgb": test_lgb, "test_cb": test_cb,
        "lgb_mape": lgb_mape, "cb_mape": cb_mape,
        "best_log_mape": best_log_mape, "best_log_w": best_log_w,
        "best_real_mape": best_real_mape, "best_real_w": best_real_w,
        "final_mape": final_mape, "blend_type": blend_type,
        "y_log": y_log,
    }, f)
